...
void printM(map<char,int> a){
	for(map<char,int>::iterator i1 = a.begin(); i1 != a.end() ; i1++){
		cout << '(' << i1->first << ","  << i1->second << ")" << endl;
	}
}

char pickOdd(map<char,int> &q){
	for(map<char,int>::iterator i1 = q.begin(); i1 != q.end() ; i1++){
		if(i1 -> second % 2 == 1){
			i1->second--;
...
int main(){
	...
	for(map<char,int>::iterator i1 = myMap.begin(); i1 != myMap.end() ; i1++){
